...
O=61; %Number of Weeks 

MM=[5,3,3]; %Number of authorized vaccine brands per age group, [18 and up, 12-17, 05-11].

Vsup=[30667459 10934225 4124719 16867197 19914821]';  %Available Vaccine Supplies at the start of simulation. In order - [Pfizer Moderna Janssen Astrazeneca Sinovac] 
...
eb=[.770;.721;.54;.556;0.15];     %Booster Effectives Against Omicron
eww=[.53;.512;.37;.286;.004];
ebb=[.770;.721;.540;.556;.15];
M=5;
f=zeros(M*O*N,1);
fb=f; fbb=fb;
for k=1:O
...
eb=[.479;0.477;0];      %Booster Effectives Against Omicron ; Only Pfizer and Moderna is authorized for booster
eww=[.41;.435;0];
ebb=[.479;0.477;0];
M=3;
f1=zeros(M*O*N,1);
...
eb=[0.58;0.61;0];         %Booster Effectives Against Omicron; Set to Pfizer and Moderna authorized for booster
eww=[0.24;0.33;0];
ebb=[0.58;0.61;0];
M=3;
f2=zeros(M*O*N,1);
...
M=5;
[~, ~, raw]=xlsread('Constraint_Data_Pop_Oct31.xlsx');
...
b3=[];
M=5;
[~, ~, raw]=xlsread('Constraint_Data_Booster_Oct31.xlsx');
[row, ~]=size(raw);
...
b3=[];
M=5;
[~, ~, raw]=xlsread('Constraint_Data_SecondBooster_Oct31.xlsx');
[row, ~]=size(raw);
